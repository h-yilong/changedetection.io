...
from .processors import (
    find_processors,
    get_parent_module,
    get_custom_watch_obj_for_processor,
)
...
try:
    ...
    logger.warning(
        f"Unable to set locale {default_locale}, locale is not installed maybe?"
    )
...
def login_optionally_required(func):
    ...
    def decorated_view(*args, **kwargs):
        ...
        # Permitted
        if (
            request.endpoint == "static_content"
            and request.view_args["group"] == "styles"
        ):
            return func(*args, **kwargs)
        # Permitted
        elif request.endpoint == "diff_history_page" and datastore.data["settings"][
            "application"
...
def changedetection_app(config=None, datastore_o=None):
    ...
    watch_api.add_resource(
        api_v1.Import, "/api/v1/import", resource_class_kwargs={"datastore": datastore}
    )
    ...
    def rss():
        ...
        for uuid, tag in (
            datastore.data["settings"]["application"].get("tags", {}).items()
        ):
        ...
        for uuid, watch in datastore.data["watching"].items():
            ...
            if datastore.data["settings"]["application"].get(
                "rss_hide_muted_watches"
            ) and watch.get("notification_muted"):
                continue
        ...
        for watch in sorted_watches:
            ...
            if not watch.viewed:
                ...
                ext_base_url = datastore.data["settings"]["application"].get(
                    "active_base_url"
                )
                ...
                diff_link = {
                    "href": url_for(
                        "diff_history_page", uuid=watch["uuid"], _external=True
                    )
                }
                ...
                watch_title = (
                    watch.get("title") if watch.get("title") else watch.get("url")
                )
                ...
                content = jinja_render(
                    template_str=rss_template,
    ...
    def index():
        ...
        if active_tag_req:
            for uuid, tag in (
                ...
                if (
                    active_tag_req == tag.get("title", "").lower().strip()
                    or active_tag_req == uuid
                ):
                    active_tag = tag
        ...
        errored_count = 0
        search_q = (
            request.args.get("q").strip().lower() if request.args.get("q") else False
        )
        for uuid, watch in datastore.data["watching"].items():
            ...
            if search_q:
                if (
                    watch.get("title") and search_q in watch.get("title").lower()
                ) or search_q in watch.get("url", "").lower():
                    sorted_watches.append(watch)
                elif (
                    watch.get("last_error")
                    and search_q in watch.get("last_error").lower()
                ):
                    sorted_watches.append(watch)
            else:
                sorted_watches.append(watch)
        ...
        output = render_template(
            ...
            sort_order=(
                ...
                else request.cookies.get("order")
            ),
    ...
    def ajax_callback_send_notification_test(watch_uuid=None):
        ...
        if (
            not watch_uuid
            and (is_global_settings_form or is_group_settings_form)
        ...
        if not notification_urls:
            logger.debug(
                "Test notification - Trying by group/tag in the edit form if available"
            )
            ...
            if request.form.get("tags") and request.form["tags"].strip():
                for k in request.form["tags"].split(","):
                    ...
                    notification_urls = (
                        tag.get("notifications_urls")
                        if tag and tag.get("notifications_urls")
                        else None
                    )

        if (
            not notification_urls
            and not is_global_settings_form
            and not is_group_settings_form
        ):
            # In the global settings, use only what is typed currently in the text box
            logger.debug(
                "Test notification - Trying by global system settings notifications"
            )
            if datastore.data["settings"]["application"].get("notification_urls"):
                notification_urls = datastore.data["settings"]["application"][
                    "notification_urls"
                ]
        ...
        try:
            ...
            n_object = {
                "watch_url": request.form.get(
                    "window_url", "https://changedetection.io"
                ),
            ...
            if (
                "notification_format" in request.form
                and request.form["notification_format"].strip()
            ):
                n_object["notification_format"] = request.form.get(
                    "notification_format", ""
                ).strip()

            if (
                "notification_title" in request.form
                and request.form["notification_title"].strip()
            ):
                n_object["notification_title"] = request.form.get(
                    "notification_title", ""
                ).strip()

            if (
                "notification_body" in request.form
                and request.form["notification_body"].strip()
            ):
                n_object["notification_body"] = request.form.get(
                    "notification_body", ""
                ).strip()
            ...
            new_worker.queue_notification_for_watch(
                notification_q=notification_q, n_object=n_object, watch=watch
            )
    ...
    def _watch_has_tag_options_set(watch):
        ...
        for tag_uuid, tag in (
            ...
            if tag_uuid in watch.get("tags", []) and (
                tag.get("include_filters") or tag.get("subtractive_selectors")
            ):
                return True
    ...
    def edit_page(uuid):
        ...
        if datastore.proxy_list is not None:  # When enabled
            ...
            if default["proxy"] is None or not any(
                default["proxy"] in tup for tup in datastore.proxy_list
            ):
                default["proxy"] = ""
        ...
        processor_classes = next(
            (tpl for tpl in find_processors() if tpl[1] == processor_name), None
        )
        if not processor_classes:
            flash(
                f"Cannot load the edit form for processor/plugin '{processor_classes[1]}', plugin missing?",
                "error",
            )
            return redirect(url_for("index"))
        ...
        if request.method == "POST" and form.validate():
            ...
            if datastore.data["watching"][uuid].get("processor") != form.data.get(
                "processor"
            ):
                datastore.data["watching"][uuid].clear_watch()
                flash("Reset watch history due to change of processor")

            extra_update_obj = {"consecutive_filter_failures": 0, "last_error": False}
            ...
            if (
                "filter_text_added" in form.data
                and not form.data.get("filter_text_added")
                and "filter_text_replaced" in form.data
                and not form.data.get("filter_text_replaced")
                and "filter_text_removed" in form.data
                and not form.data.get("filter_text_removed")
            ):
            ...
            datastore.data["watching"][uuid] = watch_class(
                datastore_path=datastore_o.datastore_path,
            ...
            if (
                (watch.get("fetch_backend") == "system" and system_uses_webdriver)
                or watch.get("fetch_backend") == "html_webdriver"
            ...
            visualselector_enabled = (
                os.getenv("PLAYWRIGHT_DRIVER_URL", False) and is_html_webdriver
            )
            template_args = {
                ...
                "has_extra_headers_file": len(
                    datastore.get_all_headers_in_textfile_for_watch(uuid=uuid)
                )
                > 0,
                ...
                "timezone_default_config": datastore.data["settings"][
                    "application"
                ].get("timezone"),
            ...
            if form.extra_form_content():
                ...
                templates_dir = str(
                    importlib.resources.files("changedetectionio").joinpath("templates")
                )
            ...
            output = render_template(
                "edit.html",
                extra_tab_content=(
                    form.extra_tab_content() if form.extra_tab_content() else None
                ),
    ...
    def settings_page():
        ...
        if datastore.proxy_list is not None:
            ...
            default["requests"]["proxy"] = (
                system_proxy if system_proxy is not None else available_proxies[0]
            )
        ...
        if request.method == "POST":
            ...
            if form.validate():
                ...
                if not os.getenv("SALTED_PASS", False) and len(
                    form.application.form.password.encrypted_password
                ):
        ...
        output = render_template(
            ...
            min_system_recheck_seconds=int(
                os.getenv("MINIMUM_SECONDS_RECHECK_TIME", 3)
            ),
            settings_application=datastore.data["settings"]["application"],
            timezone_default_config=datastore.data["settings"]["application"].get(
                "timezone"
            ),
            utc_time=utc_time,
        )
    ...
    def import_page():
        ...
        if request.method == "POST":
            ...
            if request.values.get("urls") and len(request.values.get("urls").strip()):
                ...
                importer.run(
                    data=request.values.get("urls"),
                    flash=flash,
                    datastore=datastore,
                    processor=request.values.get("processor", "text_json_diff"),
                )
                for uuid in importer.new_uuids:
                    update_q.put(
                        queuedWatchMetaData.PrioritizedItem(
                            priority=1, item={"uuid": uuid}
                        )
                    )
            ...
            # Distill.io import
            if request.values.get("distill-io") and len(
                request.values.get("distill-io").strip()
            ):
                # Import and push into the queue for immediate update check
                d_importer = import_distill_io_json()
                d_importer.run(
                    data=request.values.get("distill-io"),
                    flash=flash,
                    datastore=datastore,
                )
    ...
    def diff_history_page(uuid):
        ...
        try:
            from_version_file_contents = watch.get_history_snapshot(
                dates[from_version_index]
            )
        except Exception as e:
            from_version_file_contents = (
        ...
        try:
            to_version_file_contents = watch.get_history_snapshot(
                dates[to_version_index]
            )
        except Exception as e:
        ...
        if datastore.data["settings"]["application"].get("password") or os.getenv(
            "SALTED_PASS", False
        ):
            password_enabled_and_share_is_off = not datastore.data["settings"][
                "application"
            ].get("shared_diff_access")
    ...
    def preview_page(uuid):
        ...
        if datastore.data["watching"][uuid].history_n == 0 and (
            watch.get_error_text() or watch.get_error_snapshot()
        ):
            flash(
                "Preview unavailable - No fetch/check completed or triggers not reached",
                "error",
            )
        else:
            ...
            try:
                ...
                triggered_line_numbers = html_tools.strip_ignore_text(
                    content=content, wordlist=watch["trigger_text"], mode="line numbers"
                )

            except Exception as e:
                content.append(
                    {
                        "line": f"File doesnt exist or unable to read timestamp {timestamp}",
                        "classes": "",
                    }
                )
    ...
    def static_content(group, filename):
        ...
        if group == "screenshot":
            ...
            try:
                ...
                response = make_response(
                    send_from_directory(
                        os.path.join(datastore_o.datastore_path, filename),
                        screenshot_filename,
                    )
                )
        ...
        if group == "visual_selector_data":
            ...
            try:
                ...
                if os.path.isfile(os.path.join(watch_directory, "elements.deflate")):
                    ...
                    logger.error(
                        f'Request elements.deflate at "{watch_directory}" but was notfound.'
                    )
                    abort(404)
    ...
    def watch_get_latest_html(uuid):
        ...
        if watch and watch.history.keys() and os.path.isdir(watch.watch_data_dir):
            ...
            html_fname = os.path.join(
                watch.watch_data_dir, f"{latest_filename}.html.br"
            )
            ...
            return send_file(
                buffer,
                as_attachment=True,
                download_name=f"{latest_filename}.html",
                mimetype="text/html",
            )
            ...
            for watch_uuid, watch in datastore.data["watching"].items():
                if tag in watch.get("tags", {}):
                    ...
                    if (
                        ...
                        update_q.put(
                            queuedWatchMetaData.PrioritizedItem(
                                priority=1, item={"uuid": watch_uuid}
                            )
                        )
                        i += 1
                ...
                if op_extradata and tag_uuid:
                    for uuid in uuids:
                        ...
                        if datastore.data["watching"].get(uuid):
                            ...
                            if isinstance(
                                datastore.data["watching"][uuid]["tags"], str
                            ):
                                datastore.data["watching"][uuid]["tags"] = []
    ...
    def form_share_put_watch():
        ...
        watch["ignore_text"] += datastore.data["settings"]["application"][
            "global_ignore_text"
        ]
        watch["subtractive_selectors"] += datastore.data["settings"]["application"][
        ...
        try:
            ...
            flash(
                "Could not share, something went wrong while communicating with the share server - {}".format(
                    str(e)
                ),
                "error",
            )
        ...
        if datastore.data["watching"].get(uuid):
            if mode == "exact":
                ...
                for l in selection.splitlines():
                    ...
                    datastore.data["watching"][uuid]["ignore_text"].append(
                        "/" + s + "/"
                    )
    ...
    app.register_blueprint(
        check_proxies.construct_blueprint(datastore=datastore),
        url_prefix="/check_proxy",
    )

    import changedetectionio.blueprint.backups as backups

    app.register_blueprint(
        backups.construct_blueprint(datastore), url_prefix="/backups"
    )
    ...
    if not os.getenv("GITHUB_REF", False) and not strtobool(
        os.getenv("DISABLE_VERSION_CHECK", "no")
    ):
    ...
    while not app.config.exit.is_set():
        try:
            ...
            try:
                ...
                if not n_object.get("notification_title") and datastore.data[
                    "settings"
                ]["application"].get("notification_title"):
                    n_object["notification_title"] = datastore.data["settings"][
                        "application"
                    ].get("notification_title")

                if not n_object.get("notification_format") and datastore.data[
                    "settings"
                ]["application"].get("notification_format"):
...
def ticker_thread_check_time_launch_checks():
    ...
    logger.debug(
        f"System env MINIMUM_SECONDS_RECHECK_TIME {recheck_time_minimum_seconds}"
    )
    ...
    while not app.config.exit.is_set():
        ...
        while update_q.qsize() >= 2000:
            logger.warning(
                f"Recheck watches queue size limit reached ({MAX_QUEUE_SIZE}), skipping adding more items"
            )
            time.sleep(3)
        ...
        for uuid in watch_uuid_list:
            ...
            if watch.get("time_between_check_use_default"):
                ...
                time_schedule_limit = watch.get("time_schedule_limit")
                logger.trace(
                    f"{uuid} Time scheduler - Using watch settings (not global settings)"
                )
            ...
            if time_schedule_limit and time_schedule_limit.get("enabled"):
                try:
                    ...
                    if not result:
                        logger.trace(
                            f"{uuid} Time scheduler - not within schedule skipping."
                        )
                        continue
                except Exception as e:
                    logger.error(
            ...
            if (
                ...
                if not uuid in running_uuids and uuid not in [
                    q_uuid.item["uuid"] for q_uuid in update_q.queue
                ]:
                    ...
                    if watch_proxy and watch_proxy in list(datastore.proxy_list.keys()):
                        ...
                        proxy_list_reuse_time_minimum = int(
                            datastore.proxy_list.get(watch_proxy, {}).get(
                                "reuse_time_minimum", 0
                            )
                        )
                    ...
                    update_q.put(
                        queuedWatchMetaData.PrioritizedItem(
                            priority=priority, item={"uuid": uuid}
                        )
                    )
