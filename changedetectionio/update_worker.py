...
class update_worker(threading.Thread):
    ...
    def queue_notification_for_watch(self, notification_q, n_object, watch):
        ...
        if len(dates):
            ...
            snapshot_contents = (
                "No snapshot/history available, the watch should fetch at least once."
            )
        ...
        n_object.update(
            {
                ...
                "diff": diff.render_diff(
                    prev_snapshot, current_snapshot, line_feed_sep=line_feed_sep
                ),
                "diff_added": diff.render_diff(
                    prev_snapshot,
                    current_snapshot,
                ...
                "diff_patch": diff.render_diff(
                    prev_snapshot,
                    current_snapshot,
                    line_feed_sep=line_feed_sep,
                    patch_format=True,
                ),
                "diff_removed": diff.render_diff(
                    prev_snapshot,
                    current_snapshot,
                    include_added=False,
                    line_feed_sep=line_feed_sep,
                ...
                "screenshot": (
                    watch.get_screenshot()
                    if watch and watch.get("notification_screenshot")
                    else None
                ),
        ...
        logger.trace(
            f"Main rendered notification placeholders (diff_added etc) calculated in {time.time()-now:.3f}s"
        )
    ...
    def _check_cascading_vars(self, var_name, watch):
        ...
        if v and not watch.get("notification_muted"):
            if (
                var_name == "notification_format"
                and v == default_notification_format_for_watch
            ):
                return self.datastore.data["settings"]["application"].get(
                    "notification_format"
                )
    ...
    def send_content_changed_notification(self, watch_uuid):
        ...
        # Prefer - Individual watch settings > Tag settings >  Global settings (in that order)
        n_object["notification_urls"] = self._check_cascading_vars(
            "notification_urls", watch
        )
        n_object["notification_title"] = self._check_cascading_vars(
            "notification_title", watch
        )
        n_object["notification_body"] = self._check_cascading_vars(
            "notification_body", watch
        )
        n_object["notification_format"] = self._check_cascading_vars(
            "notification_format", watch
        )
        ...
        if n_object and n_object.get("notification_urls"):
            ...
            self.datastore.update_watch(
                uuid=watch_uuid, update_obj={"notification_alert_count": count}
            )

            self.queue_notification_for_watch(
                notification_q=self.notification_q, n_object=n_object, watch=watch
            )
    ...
    def send_filter_failure_notification(self, watch_uuid):
        ...
        n_object = {
            ...
            "notification_body": "Your configured CSS/xPath filters of '{}' for {{{{watch_url}}}} did not appear on the page after {} attempts, did the page change layout?\n\nLink: {{{{base_url}}}}/edit/{{{{watch_uuid}}}}\n\nThanks - Your omniscient changedetection.io installation :)\n".format(
                ", ".join(watch["include_filters"]), threshold
            ),
        ...
        if len(watch["notification_urls"]):
            ...
            n_object["notification_urls"] = self.datastore.data["settings"][
                "application"
            ]["notification_urls"]
        ...
        if "notification_urls" in n_object:
            n_object.update(
                {"watch_url": watch["url"], "uuid": watch_uuid, "screenshot": None}
            )
            self.notification_q.put(n_object)
            logger.debug(f"Sent filter not found notification for {watch_uuid}")
        else:
            logger.debug(
                f"NOT sending filter not found notification for {watch_uuid} - no notification URLs"
            )
    ...
    def send_step_failure_notification(self, watch_uuid, step_n):
        ...
        # Only prepare to notify if the rules above matched
        if "notification_urls" in n_object:
            n_object.update({"watch_url": watch["url"], "uuid": watch_uuid})
            self.notification_q.put(n_object)
    ...
    def run(self):
        ...
        while not self.app.config.exit.is_set():
            ...
            try:
                ...
                if uuid in list(
                    self.datastore.data["watching"].keys()
                ) and self.datastore.data["watching"][uuid].get("url"):
                    ...
                    logger.info(
                        f"Processing watch UUID {uuid} Priority {queued_item_data.priority} URL {watch['url']}"
                    )
                    now = time.time()
                    ...
                    try:
                        ...
                        processor_module_name = (
                            f"changedetectionio.processors.{processor}.processor"
                        )
                        try:
                        ...
                        update_handler = processor_module.perform_site_check(
                            datastore=self.datastore, watch_uuid=uuid
                        )

                        update_handler.call_browser()

                        changed_detected, update_obj, contents = (
                    ...
                    #                            raise Exception("Error - returned data from the fetch handler SHOULD be bytes")
                        ...
                        logger.critical(
                            f"File permission error updating file, watch: {uuid}"
                        )
                    ...
                    # A generic other-exception thrown by processors
                        ...
                        self.datastore.update_watch(
                            uuid=uuid, update_obj={"last_error": e.message}
                        )
                        process_changedetection_results = False
                        ...
                        if e.has_filters:
                            ...
                            has_img = html_tools.include_filters(
                                include_filters="img", html_content=e.html_content
                            )
                            if has_img:
                        ...
                        if e.screenshot:
                            watch.save_screenshot(
                                screenshot=e.screenshot, as_error=True
                            )
                        ...
                        self.datastore.update_watch(
                            uuid=uuid, update_obj={"last_error": err_text}
                        )
                        process_changedetection_results = False
                        ...
                        # Filter wasn't found, but we should still update the visual selector so that they can have a chance to set it up again
                        if e.screenshot:
                            watch.save_screenshot(screenshot=e.screenshot)
                        ...
                        if watch.get("filter_failure_notification_send", False):
                            ...
                            threshold = self.datastore.data["settings"][
                                "application"
                            ].get("filter_failure_notification_threshold_attempts", 0)
                            logger.debug(
                                f"Filter for {uuid} not found, consecutive_filter_failures: {c} of threshold {threshold}"
                            )
                            if c >= threshold:
                                if not watch.get("notification_muted"):
                                    logger.debug(
                                        f"Sending filter failed notification for {uuid}"
                                    )
                            ...
                            self.datastore.update_watch(
                                uuid=uuid, update_obj={"consecutive_filter_failures": c}
                            )
                        else:
                            logger.trace(
                                f"{uuid} - filter_failure_notification_send not enabled, skipping"
                            )

                        process_changedetection_results = False

                    except (
                        content_fetchers_exceptions.checksumFromPreviousCheckWasTheSame
                    ) as e:
                        ...
                        self.datastore.update_watch(
                            uuid=uuid, update_obj={"last_error": e.msg}
                        )
                        process_changedetection_results = False
                        ...
                        # Generally enough info for TimeoutError (couldn't locate the element after default seconds)
                        ...
                        logger.debug(
                            f"BrowserSteps exception at step {error_step} {str(e.original_e)}"
                        )
                        ...
                        if watch.get("filter_failure_notification_send", False):
                            ...
                            logger.error(
                                f"Step for {uuid} not found, consecutive_filter_failures: {c}"
                            )
                        ...
                        err_text = (
                            "Error running JS Actions - Page request - " + e.message
                        )
                        if e.screenshot:
                            watch.save_screenshot(
                                screenshot=e.screenshot, as_error=True
                        ...
                        err_text = "Page request from server didn't respond correctly"
                        if e.message:
                        ...
                        content_fetchers_exceptions.BrowserStepsInUnsupportedFetcher
                    ) as e:
                        ...
                        process_changedetection_results = False
                        logger.error(
                            f"Exception (BrowserStepsInUnsupportedFetcher) reached processing watch UUID: {uuid}"
                        )
                        ...
                        logger.error(str(e))
                        self.datastore.update_watch(
                            uuid=uuid, update_obj={"last_error": "Exception: " + str(e)}
                        )
                        # Other serious error
                        process_changedetection_results = False
                        ...
                        update_obj["content-type"] = (
                            update_handler.fetcher.get_all_headers()
                            .get("content-type", "")
                            .lower()
                        )
                            ...
                            if not watch["title"] or not len(watch["title"]):
                                try:
                                    update_obj["title"] = html_tools.extract_element(
                                        find="title",
                                        html_content=update_handler.fetcher.content,
                                    )
                                    logger.info(
                                        f"UUID: {uuid} Extract <title> updated title to '{update_obj['title']}"
                                    )
                                except Exception as e:
                                    logger.warning(
                                        f"UUID: {uuid} Extract <title> as watch title was enabled, but couldn't find a <title>."
                                    )
                        ...
                        try:
                            ...
                            if changed_detected or not watch.history_n:
                                ...
                                if update_handler.screenshot:
                                    watch.save_screenshot(
                                        screenshot=update_handler.screenshot
                                    )

                                if update_handler.xpath_data:
                                    watch.save_xpath_data(
                                        data=update_handler.xpath_data
                                    )

                                # Small hack so that we sleep just enough to allow 1 second  between history snapshots
                                # this is because history.txt indexes/keys snapshots by epoch seconds and we dont want dupe keys

                                if watch.newest_history_key and int(timestamp) == int(
                                    watch.newest_history_key
                                ):
                                    logger.warning(
                                ...
                                if update_handler.fetcher.content:
                                    watch.save_last_fetched_html(
                                        contents=update_handler.fetcher.content,
                                        timestamp=timestamp,
                                    )
                                ...
                                if watch.history_n >= 2:
                                    logger.info(
                                        f"Change detected in UUID {uuid} - {watch['url']}"
                                    )
                                    if not watch.get("notification_muted"):
                            ...
                            logger.critical(
                                "!!!! Exception in update_worker while processing process_changedetection_results !!!"
                            )
                            logger.critical(str(e))
                            self.datastore.update_watch(
                    ...
                    try:
                        server_header = (
                            update_handler.fetcher.headers.get("server", "")
                            .strip()
                            .lower()[:255]
                        )
                        self.datastore.update_watch(
                            uuid=uuid, update_obj={"remote_server_reply": server_header}
                        )
                    except Exception as e:
                        pass
