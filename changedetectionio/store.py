...
class ChangeDetectionStore:
    ...
    def __init__(
        ...
        try:
            ...
            with open(self.json_store_path) as json_file:
                ...
                for uuid, tag in self.__data["settings"]["application"]["tags"].items():
                    self.__data["settings"]["application"]["tags"][uuid] = (
        ...
        # First time ran, Create the datastore.
            ...
            if include_default_watches:
                logger.critical(
                    f"No JSON DB found at {self.json_store_path}, creating JSON store at {self.datastore_path}"
                )
                self.add_watch(
    ...
    def rehydrate_entity(self, uuid, entity, processor_override=None):
        ...
        if entity.get("uuid") != "text_json_diff":
            logger.trace(
                f"Loading Watch object '{watch_class.__module__}.{watch_class.__name__}' for UUID {uuid}"
            )
    ...
    def add_watch(
        ...
        if url.startswith("https://changedetection.io/share/"):
            try:
                ...
                logger.error(
                    f"Error fetching metadata for shared watch link {url} {str(e)}"
                )
    ...
    def sync_to_json(self):
        ...
        try:
            ...
            time.sleep(15)
            logger.error(
                f"! Data changed when writing to JSON, trying again.. {str(e)}"
            )
            self.sync_to_json()
            return
        else:
            ...
            try:
                ...
                logger.error(
                    f"Error writing JSON!! (Main JSON file save was skipped) : {str(e)}"
                )
    ...
    def tag_exists_by_name(self, tag_name):
        ...
        return next(
            (v for v in tags if v.get("title", "").lower() == tag_name.lower()), None
        )
    ...
    def get_unique_notification_tokens_available(self):
        ...
        for watch_uuid, watch in self.__data["watching"].items():
            ...
            if processor not in watch_processors_checked:
                extra_notification_tokens.update(
    ...
    def get_unique_notification_token_placeholders_available(self):
        ...
        for watch_uuid, watch in self.__data["watching"].items():
            ...
            if processor not in watch_processors_checked:
                extra_notification_tokens += (
                    watch.extra_notification_token_placeholder_info()
                )
                watch_processors_checked.add(processor)
    ...
    def update_1(self):
        if self.data["settings"]["requests"].get("minutes_between_check"):
            self.data["settings"]["requests"]["time_between_check"]["minutes"] = (
        ...
        for uuid, watch in self.data["watching"].items():
            if "minutes_between_check" in watch:
                ...
                if watch.get("minutes_between_check", False):
                    self.data["watching"][uuid]["time_between_check"]["minutes"] = (
            ...
            if watch.get("history", False):
                ...
                if len(history):
                    ...
                    if os.path.exists(target_path):
                        ...
                        logger.warning(
                            f"Datastore history directory {target_path} does not exist, skipping history import."
                        )
    ...
    def update_5(self):
        ...
        current_system_body = self.data["settings"]["application"][
            "notification_body"
        ].translate(TRANSLATE_WHITESPACE_TABLE)
        ...
        for uuid, watch in self.data["watching"].items():
            try:
                ...
                if (
                    watch_body
                    and watch_body.translate(TRANSLATE_WHITESPACE_TABLE)
                    == current_system_body
                ):
                ...
                if (
                    watch_title
                    and watch_title.translate(TRANSLATE_WHITESPACE_TABLE)
    ...
    def update_15(self):
        for uuid in self.__data["watching"]:
            if (
                self.__data["watching"][uuid]["time_between_check"]
                ...
                for value in self.__data["watching"][uuid][
                    "time_between_check"
                ].values()
            ):
                self.__data["watching"][uuid]["time_between_check_use_default"] = True
    ...
    def update_18(self):
        for uuid, watch in self.data["watching"].items():
            if not watch.get("restock_settings"):
                ...
                self.data["watching"][uuid]["restock_settings"] = {
                    "follow_price_changes": True
                }

            # Migrate and cleanoff old value
            self.data["watching"][uuid]["restock_settings"]["in_stock_processing"] = (
                "in_stock_only" if watch.get("in_stock_only") else "all_changes"
            )
            ...
            if os.path.exists(json_path):
                with open(json_path, "rb") as f_j:
                    with open(deflate_path, "wb") as f_d:
                        logger.debug(
                            f"Compressing {str(json_path)} to {str(deflate_path)}.."
                        )
