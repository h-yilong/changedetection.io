$(document).ready(function () {
  ...
  $('#browser_steps-0-operation option[value="Goto site"]').prop(
    "selected",
    "selected",
  );
  ...
  function set_first_gotosite_disabled() {
    $("#browser_steps >li:first-child select")
      .val("Goto site")
  ...
  setInterval(() => {
    if (browser_interface_seconds_remaining >= 1) {
      document.getElementById("browser-seconds-remaining").innerText =
        browser_interface_seconds_remaining + " seconds remaining in session";
  ...
  $("#browsersteps-img").bind("load", function () {
    ...
    $("#browsersteps-selector-canvas").bind("mousedown", function (e) {
      ...
      e.preventDefault();
      last_click_xy = {
        x: parseInt((1 / x_scale) * e.offsetX),
        y: parseInt((1 / y_scale) * e.offsetY),
      };
      ...
      function (e) {
        ...
        if (possible_elements.length) {
          possible_elements.forEach(function (item, index) {
            if (
              ["a", "input", "textarea", "button"].includes(item["tagName"])
            ) {
    ...
    if (selected_in_xpath_list !== false) {
      ...
      if (x && first_available.length) {
        ...
        if (
          ...
          $('input[placeholder="Value"]', first_available)
            .addClass("ok")
            .click()
            .focus();
  ...
  function start() {
    ...
    $.ajax({
      ...
      .done(function (data) {
        ...
        console.log(
          "Got startup response, requesting Goto-Site (first) step fake click",
        );
  ...
  function set_greyed_state() {
    $("ul#browser_steps select")
      .not('option:selected[value="Choose one"]')
      .closest("li")
      .removeClass("empty");
    $('ul#browser_steps select option:selected[value="Choose one"]')
      .closest("li")
      .addClass("empty");
  }
  ...
  $("ul#browser_steps li .control .show-screenshot").click(function (element) {
    ...
    const url =
      browser_steps_fetch_screenshot_image_url + `&step_n=${step_n}&type=${t}`;
  ...
  if (browser_steps_last_error_step) {
    $(
      "ul#browser_steps>li:nth-child(" + browser_steps_last_error_step + ")",
    ).addClass("browser-step-with-error");
  }
