...
def get_itemprop_availability(html_content) -> Restock:
    ...
    now = time.time()
    # todo: 如果 url 包含 woolworths (regEx?)，定制一个算法
    ...
    try:
        ...
        logger.warning(
            f"Unable to extract data, document parsing with extruct failed with {type(e).__name__} - {str(e)}"
        )
        return Restock()

    logger.trace(
        f"Extruct basic extract of all metadata done in {time.time() - now:.3f}s"
    )
    ...
    if data:
        ...
        image_result = image_parse.find(data)

        if image_result:
            value["image"] = image_result[0].value
        ...
        if price_result:
            ...
            if len(price_result) > 1 and len(price_result) > 1:
                ...
                logger.warning(
                    f"More than one price found {price_result}, throwing exception, cant use this plugin."
                )
        ...
        if not value.get("price") or value.get("availability"):
            logger.debug(
                f"Alternatively digging through OpenGraph properties for restock/price info.."
            )
            jsonpath_expr = parse("$..properties")

            for match in jsonpath_expr.find(data):
                if not value.get("price"):
                    value["price"] = _search_prop_by_value(
                        [match.value], "price:amount"
                    )
                if not value.get("image"):
                    value["image"] = _search_prop_by_value(
                        [match.value], "image:secure_url"
                    )
                if not value.get("image"):
                    value["image"] = _search_prop_by_value([match.value], "image")
                if not value.get("availability"):
                    value["availability"] = _search_prop_by_value(
                        [match.value], "product:availability"
                    )
                if not value.get("currency"):
                    value["currency"] = _search_prop_by_value(
                        [match.value], "price:currency"
                    )
...
class perform_site_check(difference_detection_processor):
    ...
    def run_changedetection(self, watch):
        ...
        if not len(text):
            ...
            raise ReplyWithContentButNoText(
                url=watch.link,
        ...
        for tag_uuid in watch.get("tags"):
            ...
            if tag.get("overrides_watch"):
                ...
                logger.info(
                    f"Watch {watch.get('uuid')} - Tag '{tag.get('title')}' selected for restock settings override"
                )
                break
        ...
        if itemprop_availability.get("price") or itemprop_availability.get(
            "availability"
        ):
            ...
            if itemprop_availability.get("availability"):
                # @todo: Configurable?
                if any(
                    substring.lower() in itemprop_availability["availability"].lower()
                    for substring in [
                        "instock",
                        "instoreonly",
                        "limitedavailability",
        ...
        if (
            itemprop_availability
            and itemprop_availability.get("price")
        ...
        if (
            not self.fetcher.instock_data
            and not itemprop_availability.get("availability")
            and not itemprop_availability.get("price")
        ):
            raise ProcessorException(
        ...
        logger.debug(
            f"self.fetcher.instock_data is - '{self.fetcher.instock_data}' and itemprop_availability.get('availability') is {itemprop_availability.get('availability')}"
        )
        # Nothing automatic in microdata found, revert to scraping the page
        if (
            self.fetcher.instock_data
            ...
            update_obj["restock"]["in_stock"] = (
                True if self.fetcher.instock_data == "Possibly in stock" else False
            )
            logger.debug(
                f"Watch UUID {watch.get('uuid')} restock check returned instock_data - '{self.fetcher.instock_data}' from JS scraper."
            )
        ...
        if (
            ...
            if update_obj["restock"].get("in_stock"):
                ...
                logger.warning(
                    f"Setting instock to FALSE, scraper found '{self.fetcher.instock_data}' in the body but metadata reported not-in-stock"
                )
                update_obj["restock"]["in_stock"] = False

        # What we store in the snapshot
        price = (
            update_obj.get("restock").get("price")
            if update_obj.get("restock").get("price")
            else ""
        )
        snapshot_content = (
            f"In Stock: {update_obj.get('restock').get('in_stock')} - Price: {price}"
        )
        ...
        logger.debug(
            f"Watch UUID {watch.get('uuid')} restock check - Previous MD5: {watch.get('previous_md5')}, Fetched MD5 {fetched_md5}"
        )

        # out of stock -> back in stock only?
        if watch.get("restock") and watch["restock"].get("in_stock") != update_obj[
            "restock"
        ].get("in_stock"):
            # Yes if we only care about it going to instock, AND we are in stock
            if (
                restock_settings.get("in_stock_processing") == "in_stock_only"
                and update_obj["restock"]["in_stock"]
            ):
        ...
        if (
            restock_settings.get("follow_price_changes")
            and watch.get("restock")
            and update_obj.get("restock")
            and update_obj["restock"].get("price")
        ):
            ...
            if update_obj.get("restock") and update_obj["restock"].get("price"):
                ...
                if update_obj["restock"].get("price"):
                    min_limit = (
                        float(restock_settings.get("price_change_min"))
                        if restock_settings.get("price_change_min")
                        else None
                    )
                    max_limit = (
                        float(restock_settings.get("price_change_max"))
                        if restock_settings.get("price_change_max")
                        else None
                    )

                    price = float(update_obj["restock"].get("price"))
                    logger.debug(
                        f"{watch.get('uuid')} after float conversion - Min limit: '{min_limit}' Max limit: '{max_limit}' Price: '{price}'"
                    )
                    if min_limit or max_limit:
                        if is_between(number=price, lower=min_limit, upper=max_limit):
                            ...
                            logger.trace(
                                f"{watch.get('uuid')} {price} is between {min_limit} and {max_limit}, nothing to check, forcing changed_detected = False (was {changed_detected})"
                            )
                            changed_detected = False
                        else:
                            logger.trace(
                                f"{watch.get('uuid')} {price} is between {min_limit} and {max_limit}, continuing normal comparison"
                            )

                    # Price comparison by %
                    if (
                        watch["restock"].get("original_price")
                        and changed_detected
                        and restock_settings.get("price_change_threshold_percent")
                    ):
                        previous_price = float(watch["restock"].get("original_price"))
                        pc = float(
                            restock_settings.get("price_change_threshold_percent")
                        )
                        change = abs((price - previous_price) / previous_price * 100)
                        if change and change <= pc:
                            logger.debug(
                                f"{watch.get('uuid')} Override change-detected to FALSE because % threshold ({pc}%) was {change:.3f}%"
                            )
                            changed_detected = False
                        else:
                            logger.debug(
                                f"{watch.get('uuid')} Price change was {change:.3f}% , (threshold {pc}%)"
                            )
