...
def test_itemprop_price_change(client, live_server):
    ...
    res = client.post(
        ...
        data={
            "restock_settings-follow_price_changes": "",
            "url": test_url,
            "tags": "",
            "headers": "",
...
def _run_test_minmax_limit(client, extra_watch_edit_form):
    ...
    res = client.post(
        url_for("edit_page", uuid="first"), data=data, follow_redirects=True
    )
    assert b"Updated watch." in res.data
    wait_for_all_checks(client)
...
def test_restock_itemprop_with_tag(client, live_server):
    ...
    res = client.post(
        url_for("tags.form_tag_add"), data={"name": "test-tag"}, follow_redirects=True
    )
...
def test_change_with_notification_values(client, live_server):
    ...
    notification_url = url_for("test_notification_endpoint", _external=True).replace(
        "http", "json"
    )
    ...
    res = client.post(
        url_for("ajax_callback_send_notification_test", watch_uuid=uuid),
        data={},
        follow_redirects=True,
    )
    time.sleep(5)
...
def test_data_sanity(client, live_server):
    ...
    res = client.get(url_for("index"))
    assert (
        str(res.data.decode()).count("950.95") == 1
    ), "Price should only show once (for the watch added, no other watches yet)"
...
def test_special_prop_examples(client, live_server):
    ...
    check_path = os.path.join(
        os.path.dirname(__file__), "itemprop_test_examples", "*.txt"
    )
