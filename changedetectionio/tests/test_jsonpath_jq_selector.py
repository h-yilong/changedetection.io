...
<h4>ok</h4>
    ...
    if jq_support:
        ...
        with pytest.raises(html_tools.JSONNotFound) as e_info:
            html_tools.extract_json_as_string(
                "COMPLETE GIBBERISH, NO JSON!", "jqraw:.id"
            )
...
def test_check_jq_ext_filter(client, live_server, measure_memory_usage):
    if jq_support:
        check_json_ext_filter(
            'jq:.[] | select(.status | contains("Sold"))', client, live_server
        )
