...
os.environ["PLAYWRIGHT_DRIVER_URL"] = "ws://localhost:3333"
    ...
    def __init__(self, proxy_override=None, custom_browser_connection_url=None):
        ...
        if custom_browser_connection_url:
            ...
            self.browser_connection_url = os.getenv(
                "PLAYWRIGHT_DRIVER_URL", "ws://localhost:3333"
            ).strip('"')
    ...
    def run(
        ...
        with sync_playwright() as p:
            ...
            browser = browser_type.connect_over_cdp(
                self.browser_connection_url, timeout=60000
            )
            ...
            from changedetectionio.blueprint.browser_steps.browser_steps import (
            ...
            if response is None:
                ...
                browser.close()
                logger.debug(
                    "Content Fetcher > Response object from the browser communication was none"
                )
                raise EmptyReply(url=url, status_code=None)

            try:
                if self.webdriver_js_execute_code is not None and len(
                    self.webdriver_js_execute_code
                ):
                    browsersteps_interface.action_execute_js(
                ...
                logger.debug(
                    f"Content Fetcher > Other exception when executing custom JS code {str(e)}"
                )
                context.close()
                browser.close()
            ...
            try:
                ...
                logger.critical(
                    f"Response from the browser/Playwright did not have a status_code! Response follows."
                )
            ...
            if not empty_pages_are_a_change and len(self.page.content().strip()) == 0:
                logger.debug(
                    "Content Fetcher > Content was empty, empty_pages_are_a_change = False"
                )
                context.close()
                browser.close()
            ...
            if current_include_filters is not None:
                self.page.evaluate(
                    "var include_filters={}".format(json.dumps(current_include_filters))
                )
            else:
            ...
            self.xpath_data = self.page.evaluate(
                "async () => {"
                + self.xpath_element_js.replace(
                    "%ELEMENTS%", visualselector_xpath_selectors
                )
                + "}"
            )
